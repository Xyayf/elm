...
import { checkout, getAddressList, placeOrders } from '@/api/getData.js'
...
export default {
  ...
  computed: {
    ...mapState({
      remarkText: state => state.shopCart.remarktext,
      inputText: state => state.shopCart.inputremark,
      geohash: state => state.location.geohash,
      userInfo: state => state.user.userInfo,
      choosedAddress: state => state.user.addressRes
    ...
    async initAddress () {
      if (this.userInfo && this.userInfo.user_id) {
        ...
        if (addressRes instanceof Array && addressRes.length) {
          this.$store.commit({ address: addressRes[0], index: 0 })
        }
      }
    },
    // 显示付款方式
